...
dotnet_sdk::
	...
	cp README.md LICENSE ${PACKDIR}/dotnet/ && \
	cd ${PACKDIR}/dotnet/&& \
...
nodejs_sdk:: NODE_PACKAGE_NAME := $(shell echo -n $(NODE_MODULE_NAME) | sed -r 's/\//\\\//g')
nodejs_sdk::
	rm -rf sdk/nodejs
	pulumi package gen-sdk $(WORKING_DIR)/bin/$(PROVIDER) --language nodejs
	cd ${PACKDIR}/nodejs/ && \
		echo $(NODE_PACKAGE_NAME) && \
		sed -i.bak 's/"name": "@pulumi.*"/"name": "$(NODE_PACKAGE_NAME)"/g' package.json && \
		rm ./package.json.bak && \
		yarn install && \
		yarn run tsc && \
